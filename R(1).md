...
- 통계적 가설 검정 : 유의 확률을 이용해 가설을 검정하는 방법을 말한다. '유의확률'은 실제로는 집단 간 차이가 없는데 우연히 차이가 있는 데이터가 추출될 확률을 의미한다. 통계 분석을 실시한 결과 유의확률이 크게 나타났다면 '집단 간 차이가 통계적으로 유의하지 않다' 고 해석한다. 이는 실제로 차이가 없더라도 우연에 의해 이 정도의 차이가 관찰될 가능성이 크다는 의미입니다.
  ...
  > 통계에서 중요한 것은 데이터를 돌리는 것보다 먼저 해당 데이터가 돌리기에 적합한지를 판별하는 것이 중요하다. 

###### t 검정 - 두 집단의 평균 비교

- 두 집단의 평균에 통계적으로 유의한 차이가 있는지 알아 볼때 사용.

- R에 내장된 t.test()를 이용해 t검정을 할 수 있다. 

- compact 자동차와 suv 자동차의 도시 연비 t 검정

  - ggplot2 패키지의 mpg 데이터를 이용해 t검정을 수행하는 방법

  - 1.  먼저 mpg 데이터를 불러와 class,cty 변수만 남긴 뒤 class 변수가 'compat' , 'suv' 자동차를 추출

       ```bash
       library(ggplot2)
       
       #mpg$class %in% c("compact","suv") : compat 과 suv 만 추출 하기 위해 
    ...
    2. t.test()를 이용행 t검정 , ~ 기호를 이용해 비교할 값인 cty(도시연비) 변수와 비교할 집단인 class(자동차 종류) 변수를 지정합니다.
    ...
    # 집단 간 분산이 같다고 가정하고 var.equal 에 T를 지정
    ...
    > 출력된 t 검정 결과에서 'p-value' 가 유의 확률을 의미한다. 일반적으로 유의 확률 5%를 판단 기준으로 삼고, 0.05 미만이면 '집단 간 차이가 통계적으로 유의하다' 고 해거한다.
    >
    > 'sample estimates' 부분을 보면 각 집단의 cty 평균이 나타난다. "compact" 20 , "suv" 는 13 이므로 "compact"의 도시 연비가 더 높다고 할 수 있다.
    ...
    ```bash
    #오토 차량과 수동 차량의 고속 연비의 차이가 있다?
    library(ggplot2)
    str(mpg)
    table(mpg$hwy)
    mpg_diff3 <- mpg[,c("hwy","trans")]
    mpg_diff3$newtrans <- ifelse(mpg_diff3$trans %in% c("manual(m5)","manual(m6)"), "MANUAL", "AUTO")
    mpg_diff3 <- mpg_diff3[,c("hwy","newtrans")]
    
    #다른 방식 
    mpg$tr <- ifelse(substr(mpg$trans,1,4) %in% "auto","auto","manual")
    
    table(mpg_diff3$newtrans)
    t.test(data = mpg_diff3, hwy ~ newtrans,var.equal = T)
    ...
    data:  hwy by newtrans
    t = -4.3683, df = 232, p-value = 1.888e-05
    alternative hypothesis: true difference in means is not equal to 0
    95 percent confidence interval:
     -5.058608 -1.913846
    sample estimates:
...
- ''상관분석'' 은 두 연속 변수가 서로 관련이 있는지 검정하는 통계 분석기법입니다.  (미래를 예측할 수 있다.) 

- 상관계수는 0~1 사이의 값을 지니고 1에 가까울수록 관련성이 크다는 것을 의미한다. 

- 실업자 수와 개인 소비 지출의 상관관계

  - ggplot2 패키지의 economics 데이터를 이용해서 uemploy(실업자 수) 와 pce(개인 소비 지출) 간체 통계적으로 유의한 상관관계가 있는지 알아보자. R에 내장된 cor.test()를 이용하면 상관분석을 할 수 있다.
    ...
    #상관관계 분석
    re <- cor.test(economics$pce,economics$unemploy)
    
    #유의관계 분석
    if(re$p.value < 0.05){#상관관계가 있으니
      result = lm(economics$pce~economics$unemploy)
      #공식을 추출 (y = -1609.1876 + 0.8273x)
      result$coefficients
    }else{
    ...
    >
    > 출력결과 'cor'이 상관계수를 의미한다. 상관계수가 양수 0.61이므로, 실업자 수과 개인 소비 지출은 한 변수가 증가하면 다른 변수가 증가는 정비례 관계임을 알 수 있다. 

    

    (그래프 결과)

    ![graph](https://user-images.githubusercontent.com/50862260/63418159-98602e00-c43d-11e9-916f-4d00bc5f87d0.JPG)
